...
def main(image_path: str = "page_new.png", dpi: int = 300, debug: bool = False):
...
def analyze_image(
    ...
    """
    Pure function version of main(). No global state. Returns structured dict.
    Writes annotated PNG + mask PNGs to out_dir.
    """
    bgr = cv2.imread(image_path)
    if bgr is None:
        raise FileNotFoundError(f"Cannot open image: {image_path}")
    h, w = bgr.shape[:2]
    color_results_raw = {}   # for draw_results()
    color_results_out = {}   # for return value
    grand_px = grand_ft = grand_m = 0.0

    for color_name in WIRE_COLORS:
        color_mask = extract_color_mask(bgr, color_name)
        wire_mask, wire_contours = filter_wire_contours(color_mask, color_name)
        mask_path = os.path.join(out_dir, f"{run_id}_mask_{color_name}.png")
        cv2.imwrite(mask_path, wire_mask)
        ...
        segments = []
        seg_results = []  # for draw_results()
        for i, cnt in enumerate(wire_contours):
            x, y, ww, hh = cv2.boundingRect(cnt)
            roi = wire_mask[y:y+hh, x:x+ww]
            pix_len = _skeleton_pixel_length(roi)
            if pix_len < MIN_WIRE_PIXEL_LENGTH:
                continue
            ft = pix_len / pixels_per_foot
            m  = pix_len / pixels_per_meter
            segments.append({"id": i+1, "pixel_length": pix_len,
                              "length_ft": ft, "length_m": m, "bbox": [x, y, ww, hh]})
            seg_results.append({"id": i+1, "bbox": (x, y, ww, hh),
                                 "pixel_length": pix_len, "length_ft": ft, "length_m": m})

        color_results_raw[color_name] = {"wire_mask": wire_mask, "results": seg_results}
        total_px = sum(s["pixel_length"] for s in segments)
        total_ft = sum(s["length_ft"]    for s in segments)
        total_m  = sum(s["length_m"]     for s in segments)
        color_results_out[color_name] = {
            "segments": segments,
            "total_pixel_length": total_px,
            "total_feet": total_ft,
            "total_meters": total_m,
        }
        grand_px += total_px; grand_ft += total_ft; grand_m += total_m

    annotated = draw_results(bgr, color_results_raw)
    ann_path = os.path.join(out_dir, f"{run_id}_annotated.png")
    ...
    return {
        ...
        "colors": color_results_out,
        "totals": {
            "wire_count": sum(len(v["segments"]) for v in color_results_out.values()),
            "pixel_length": grand_px,
            "length_feet": grand_ft,
            "length_meters": grand_m,
        },
    }
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Detect colored wires in a schematic PNG and measure lengths.")
    parser.add_argument("image", nargs="?", default="page_0.png", help="Path to the schematic PNG image")
    parser.add_argument("--dpi", type=int, default=300, help="DPI the image was converted at")
    parser.add_argument("--debug", action="store_true", help="Enable detailed debug logs and output masks")
    args = parser.parse_args()

    main(args.image, dpi=args.dpi, debug=args.debug)
